...
const submit = () => {
  ...
  client
    .passwordReset(route.query.selector as string, route.query.secret as string, password.value)
    .then(() => {
    ...
    .catch((axiosError) => {
      if (axiosError.response.status === 422) {
        ...
        topAlerts.add(
          'Invalid reset token. It may be expired. Try to request password change again.',
          'warning'
        )
      } else {
